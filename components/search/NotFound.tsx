...
export default function NotFound({ termDescription = "Resultados de busca encontrados para:", title = "Ops!", hints, searchTerm, }: SectionProps<typeof loader>) {
      ...
      <div class="flex flex-col gap-[10px] lg:flex-row lg:justify-around lg:mt-[30px] lg:mb-[60px]">
        <div class="flex flex-col px-5 py-[30px] gap-5">
          ...
          <div class="lg:w-[307px]">
            <Searchbar placeholder="Digite sua busca novamente" noContainer hide={{
            results: true,
            cleanButton: true,
        }} cardLayout={{
            discount: {
        ...
        {hints && (<div class="bg-secondary-focus p-5 flex flex-col gap-[10px] text-white rounded-[20px] lg:relative lg:justify-center lg:p-[60px] lg:rounded-r-none">
            ...
            <ul class="flex flex-col gap-3">
              {hints.hints.map((hint) => (<li class="text-xs lg:text-sm flex items-center gap-2">
                  <Icon height={10} width={10} id="ChevronRight" class="text-emphasis"/>
                  <span>{hint}</span>
                </li>))}
            </ul>
          </div>)}
