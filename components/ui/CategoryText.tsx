...
import { type SectionProps, type LoaderReturnType } from "@deco/deco";
...
function CategoryText({ category }: SectionProps<ReturnType<typeof loader>>) {
    ...
    return (<div class={`container ${textAlignment}`}>
      <h3 class="text-secondary max-w-5xl m-auto font-normal text-2xl mb-5">
        {categoryText}
      </h3>
      {html
            ? (<div dangerouslySetInnerHTML={{ __html: html }} class="text-neutral font-normal text-sm max-w-5xl m-auto pb-12"/>)
            : null}
    </div>);
}
...
export const loader = ({ categories = [] }: Props, req: Request) => {
    const category = categories.find(({ matcher }) => new URLPattern({ pathname: matcher }).test(req.url));
    return { category };
};
