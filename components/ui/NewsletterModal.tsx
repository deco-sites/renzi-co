...
// const subscribe = Runtime.create("apps/vtex/actions/newsletter/subscribe.ts");
...
function InputNewsletter({ name, placeholder, required, type }: InputNewletterProps) {
    return (<input name={name} type={type} class="input lg:h-12 h-9 px-5 join-item w-full mb-2.5 first:mt-5 border-2 border-neutral rounded-full placeholder:text-placeholder !outline-none lg:text-base text-xs" placeholder={placeholder} required={required}/>);
}
function NewsletterModal({ isOpen, form, text, modalSignExpiredDate, modalCloseExpiredDate, }: SectionProps<ReturnType<typeof loader>>) {
    ...
    const nameInput = !form?.name?.show
        ...
        : null;
    return (<>
      <dialog ref={modalRef} class="modal bg-primary-content bg-opacity-70">
        <form method="dialog" class="modal-box overflow-visible p-10">
          <div class="flex text-secondary-content justify-center items-center absolute right-2 -top-10">
            ...
            <button onClick={() => setCookieOnCloseModal("closed", modalCloseExpiredDate)} class="btn btn-sm btn-circle btn-ghost focus:outline-none" aria-label="Fechar">
          ...
          {success.value
            ...
            : (<>
                <Icon class="mx-auto mb-5 block" id="Logo" width={236} height={47}/>
                <div dangerouslySetInnerHTML={{ __html: text }} class="text-base lg:text-xl text-center text-base-100 lg:pr-0 "/>
