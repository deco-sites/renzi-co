...
function ProductShelf({ products, title, layout, cardLayout, seeMore, showPaginationArrows, }: Props) {
    ...
    return (<div class="w-full py-8 flex flex-col gap-12 lg:gap-7 lg:py-10">
      ...
      <div id={id} class="grid grid-cols-[48px_1fr_48px] px-0">
        <Slider class="carousel carousel-start gap-6 col-span-full row-start-2 row-end-5">
          {products?.map((product, index) => (<Slider.Item index={index} class="carousel-item w-[270px]">
              <ProductCard product={product} itemListName={title} layout={cardLayout}/>
            </Slider.Item>))}
        </Slider>

        <>
          <div class={`relative z-10 col-start-1 row-start-3  ${CONDITIONAL_RESPONSIVE_PARAMS[showPaginationArrows ? showPaginationArrows : "Always"]}`}>
          ...
          <div class={`relative z-10 col-start-3 row-start-3 ${CONDITIONAL_RESPONSIVE_PARAMS[showPaginationArrows ? showPaginationArrows : "Always"]}`}>
